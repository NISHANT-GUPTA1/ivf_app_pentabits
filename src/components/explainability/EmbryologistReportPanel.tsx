        ...
        <div className="space-y-3 animate-fadeIn animation-delay-100">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between items-center p-2 bg-slate-50 rounded border border-slate-200">
              ...
              <span className="font-bold text-slate-900">{Math.round(embryo.viabilityScore * 0.95)}/100</span>
            </div>
            <div className="flex justify-between items-center p-2 bg-slate-50 rounded border border-slate-200">
              <span className="text-slate-700">Fragmentation:</span>
              <span className="font-bold text-slate-900">{Math.max(0, Math.round((100 - embryo.viabilityScore) * 0.8))}%</span>
            </div>
            <div className="flex justify-between items-center p-2 bg-slate-50 rounded border border-slate-200">
              <span className="text-slate-700">Boundary Integrity:</span>
              <span className="font-bold text-slate-900">{Math.round(embryo.viabilityScore * 0.85)}/100</span>
            </div>
            <div className="flex justify-between items-center p-2 bg-slate-50 rounded border border-slate-200">
              <span className="text-slate-700">Symmetry:</span>
              <span className={`font-bold ${embryo.viabilityScore >= 75 ? 'text-green-700' : embryo.viabilityScore >= 50 ? 'text-amber-700' : 'text-red-700'}`}>
                {embryo.viabilityScore >= 75 ? 'Excellent' : embryo.viabilityScore >= 50 ? 'Good' : 'Fair'}
              </span>
            </div>
          </div>
          <button 
            onClick={() => onViewMetrics?.()}
            className="text-xs text-blue-600 hover:text-blue-700 font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-300 rounded"
          >
            View Full Metrics →
          </button>
        </div>
