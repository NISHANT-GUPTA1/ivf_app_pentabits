...
app = FastAPI(title="Embryo Viability API with Audit Trail")
...
async def predict(
    ...
    current_user: User = Depends(require_embryologist),
    ...
    try:
        ...
        print(f"[BACKEND] User: {current_user.username} ({current_user.role})")
        print(f"{'='*80}\n")
        ...
        log_ai_prediction(db, current_user, ai_log)
...
async def get_audit_logs(
    ...
    # Log audit access
    log_data = AuditLogCreate(action="AUDIT_LOG_ACCESSED", details={"filters": {
...
async def export_audit_pdf(
    ...
    elements.append(Paragraph(f"Generated on: {datetime.utcnow().isoformat()}", styles['Normal']))
