 ...
 */
const MorphometryMetrics: React.FC<MorphometryMetricsProps> = ({ embryo }) => {
  // Extract real morphometry data from explainability signals
  const getMetricFromSignals = (label: string) => {
    const signal = embryo.explainability?.signals.find(s => s.label.toLowerCase().includes(label.toLowerCase()));
    return signal ? Math.round(signal.impact * 100) : null;
  };
  ...
  const metrics = [
    {
      ...
      value: circularityValue,
      max: 100,
      interpretation: getInterpretation(circularityValue),
      description: 'Nucleus and cell shape roundness and definition',
      color: circularityValue >= 75 ? '#10B981' : circularityValue >= 60 ? '#F59E0B' : '#EF4444',
    },
    {
      name: 'Fragmentation',
      value: fragmentationValue,
      max: 100,
      interpretation: fragmentationValue <= 15 ? 'Minimal' : fragmentationValue <= 30 ? 'Moderate' : 'High',
      description: 'Cytoplasmic fragmentation and cell damage level',
      color: fragmentationValue <= 15 ? '#10B981' : fragmentationValue <= 30 ? '#F59E0B' : '#EF4444',
    },
    {
      name: 'Boundary Integrity',
      value: boundaryValue,
      max: 100,
      interpretation: getInterpretation(boundaryValue),
      description: 'Cell membrane and outline quality',
      color: boundaryValue >= 75 ? '#10B981' : boundaryValue >= 60 ? '#F59E0B' : '#EF4444',
    },
    {
      name: 'Symmetry',
      value: symmetryValue,
      max: 100,
      interpretation: getInterpretation(symmetryValue),
      description: 'Blastomere distribution uniformity',
      color: symmetryValue >= 75 ? '#10B981' : symmetryValue >= 60 ? '#F59E0B' : '#EF4444',
    },
  ];

  // Time-series data for video analysis (fallback to mock data)
  const timeSeriesData = Array.isArray(embryo.timeSeries) && embryo.timeSeries.length > 0
    ? embryo.timeSeries.map((entry) => ({
        ...entry,
