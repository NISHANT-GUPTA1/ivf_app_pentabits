...
import { EmbryoResult, FEATURE_LABELS } from '../../../types/embryo';
...
const FeatureAttributionChart: React.FC<FeatureAttributionChartProps> = ({ embryo }) => {
  const featureImportance = (embryo.featureImportance || []).map((item) => ({
    name: FEATURE_LABELS[item.feature] || item.feature.replace(/_/g, ' ').replace(/mean|std/g, '').trim(),
